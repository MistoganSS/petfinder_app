...
const EditReportPage = ({ params }) => {
    ...
    try {
      const response = await fetch(`${config.apis.animals}/${id}`, {
        ...
        body: JSON.stringify({
          ...
          ddLat: parseFloat(formData.ddLat),
          ddLon: parseFloat(formData.ddLon),
          dateLastSeen: last,
      ...
      router.push('/dashboard/report-pet')
    } catch (error) {
  ...
  return (
    <div>
      <h1 className='text-center mt-4 text-xl font-bold text-gray-900'>
        Update report
      </h1>
